...
public class LastPalletsDAO{
    ...
    public boolean remove() {
        ...
        try {
            ...
            Statement stm = conn.createStatement();
            String sql =
                    "DELETE FROM LastPallets LIMIT 1";
    ...
    public List<List<String>> getAll(){
        ...
        try {
            ...
            Statement stm = conn.createStatement();
            String sql =
                    "SELECT * FROM LastPallets";
            ResultSet rs = stm.executeQuery(sql);
            while (rs.next()) {
                res.add(Arrays.asList(rs.getString(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5)));
            }
            conn.close();
        } catch (SQLException e) {
            e.printStackTrace();
