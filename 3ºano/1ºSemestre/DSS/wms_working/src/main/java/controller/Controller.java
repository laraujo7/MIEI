...
public class Controller {
    ...
    private void dashboard(String id) throws ClassNotFoundException, IOException {
        ...
        do {
            ...
            if (input[0].equals("help")) showMethods(methods);
                ...
                callMethod(
                        Class.forName("controller." + className + "Controller"),
                        methods,
                        input,
                        id);
    ...
    private void showMethods(Method[] methods) throws ClassNotFoundException {
        for (int i = 0; i < methods.length; i++) {
            ...
            for (int j = 0; j < parameters.length; j++) {
                for (String string :
                        Arrays.asList(
                                " ", parameters[j].getType().getName().replace("java.lang.", ""))) {
                    Output.show(string);
                }
            }
            Output.showln(" ");
    ...
    private void callMethod(Class clazz, Method[] methods, String[] input, String id) {
        try {
            Object instance =
                    clazz.getConstructor(new Class[] {String.class, IWMS.class})
                            .newInstance(id, this.wms);
