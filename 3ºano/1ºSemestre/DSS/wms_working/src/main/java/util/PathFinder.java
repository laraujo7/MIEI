...
public class PathFinder {
    ...
    public PathFinder(Matrix matrix) {
        ...
        for (int i = 0; i < matrix.getDimensions().getValue(); i++) {
            ...
            this.cellDetails.add(
                    i,
                    new ArrayList<>(
                            Collections.nCopies(matrix.getDimensions().getKey(), new Cell())));
        }
        initOpened();
        initClosed();
    }
    ...
    private void searchAdjacent(Pair<Integer, Integer> initial, Pair<Integer, Integer> dest) {
        for (int i = 0; i < 8; i++) {
            ...
            Pair<Integer, Integer> adj =
                    new Pair<>(
                            initial.getKey() + direction.getKey(),
                            initial.getValue() + direction.getValue());
            ...
            if (matrix.isValid(adj.getKey(), adj.getValue())) {
                ...
                } else if (closedList.get(adj) == false
                        && this.matrix.getAtPos(adj) == Matrix.empty) {

                    int gNew = getCellDetailG(initial) + weights.get(i);
                    int hNew = (int) calculateHValue(adj.getKey(), adj.getValue(), dest);
                    int fNew = gNew + hNew;

                    if (this.getCellDetailF(adj) == Integer.MAX_VALUE
                            || getCellDetailF(adj) > fNew) {
    ...
    private Pair<Integer, Integer> mininumF_inOpen() {
        List<Pair<Pair<Integer, Integer>, Integer>> opened =
                openList.stream()
                        .map(
                                p -> {
                                    Pair<Pair<Integer, Integer>, Integer> ret =
                                            new Pair<>(p, this.getCellDetailF(p));
                                    return ret;
                                })
    ...
    private double calculateHValue(int x, int y, Pair<Integer, Integer> dest) {
        ...
        return Integer.max(
                Math.abs((x - dest.getKey()) * 10), Math.abs((y - dest.getValue()) * 10));
    }
