...
*/
    ...
    @Override
    public void authenticate(String id, String password) throws AuthenticationException {
        Admin admin = this.admins.get(id);
        if(admin == null){
            this.trucks.put(id, new Truck("truck", 0, 0, 100));
            TruckDriver truckDriver = this.truckDrivers.get(id);
            if(truckDriver == null){
                this.trucks.remove(id);
                FloorWorker floorWorker = this.floorWorkers.get(id);
                if (floorWorker == null) {
                    throw new AuthenticationException("Invalid Credentials");
                }
                else{
                    if(!floorWorker.getActive() || !floorWorker.authenticate(password)) {
                        throw new AuthenticationException("Invalid Credentials");
                    }
                ...
                if(!truckDriver.getActive() || !truckDriver.authenticate(password))
                    throw new AuthenticationException("Invalid Credentials");
            }
        }
        else{
            if(!admin.getActive() || !admin.authenticate(password))
                throw new AuthenticationException("Invalid Credentials");
        }
    }
