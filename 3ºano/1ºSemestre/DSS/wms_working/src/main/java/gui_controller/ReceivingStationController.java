...
public class ReceivingStationController extends UserController {
    ...
    private void readQRButtonHandler(ActionEvent event) {
        ...
        if (res != -1) {
            String palletID = list.get(res).getValue().get(0);
            wms.changeStatus(palletID);
            //Enviamos a palete para futuramente dividir as paletes por tipo
            wms.createNewJob(palletID, wms.chooseShelf(palletID));
